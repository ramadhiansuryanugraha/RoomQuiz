...
import unj.cs.student.StudentViewModelFactory
        ...
        if (index != -1){
            ...
            submitButton.setOnClickListener {
                if(idEditText.text.toString() == "" || nameEditText.text.toString() == ""){
                    ...
                    Toast.makeText(context, "ID dan Nama tidak boleh kosong", Toast.LENGTH_LONG).show()
                } else {
                    val student = Student(
                        id = index,
                    ...
                    val action: NavDirections =
                        unj.cs.student.StudentFormFragmentDirections.actionStudentFormFragmentToStudentListFragment()
                    view.findNavController().navigate(action)

                    Toast.makeText(context, "Data diperbaharui", Toast.LENGTH_LONG).show()
                }
            ...
            submitButton.setOnClickListener {
                if(idEditText.text.toString() == "" || nameEditText.text.toString() == ""){
                    ...
                    val action: NavDirections =
                        unj.cs.student.StudentFormFragmentDirections.actionStudentFormFragmentToStudentListFragment()
                    view.findNavController().navigate(action)
                    Toast.makeText(context, "${student.name} ditambahkan", Toast.LENGTH_LONG).show()
                }
            }
        }
    }
